  ...
  <main class="container mx-auto flex flex-col items-center gap-6">
    <div class="text-center">
      ...
      <div class="flex justify-center gap-2">
        <div>
          <span class="text-gray-400">Total Kunjungan:</span> <span class="font-semibold">{{ isLoading ? 'Loading...' :
            general?.details[name]?.count ?? '-' }}</span>
        </div>
        <div class="text-green-400">•</div>
        <div>
          <span class="text-gray-400">Kunjungan Unik:</span> <span class="font-semibold">{{ isLoading ? 'Loading...' :
            general?.details[name]?.unique ?? '-' }}</span>
        </div>
      </div>
    </div>
    ...
    <table v-if="guests" class="w-full max-w-prose border-collapse">
      <thead>
        <tr>
